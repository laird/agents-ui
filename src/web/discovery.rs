...
mod tests {
    ...
    #[test]
    fn parse_session_name_claude() {
        let result = parse_session_name("claude-myrepo");
        assert!(result.is_some());
        let (agent_type, project) = result.unwrap();
        assert!(matches!(agent_type, AgentType::Claude));
        assert_eq!(project, "myrepo");
    }

    #[test]
    fn parse_session_name_codex() {
        let result = parse_session_name("codex-agents-ui");
        assert!(result.is_some());
        let (agent_type, project) = result.unwrap();
        assert!(matches!(agent_type, AgentType::Codex));
        assert_eq!(project, "agents-ui");
    }

    #[test]
    fn parse_session_name_droid() {
        let result = parse_session_name("droid-foo");
        assert!(result.is_some());
        let (agent_type, project) = result.unwrap();
        assert!(matches!(agent_type, AgentType::Droid));
        assert_eq!(project, "foo");
    }

    #[test]
    fn parse_session_name_gemini() {
        let result = parse_session_name("gemini-bar");
        assert!(result.is_some());
        let (agent_type, project) = result.unwrap();
        assert!(matches!(agent_type, AgentType::Gemini));
        assert_eq!(project, "bar");
    }

    #[test]
    fn parse_session_name_unknown_returns_none() {
        assert!(parse_session_name("unknown-session").is_none());
        assert!(parse_session_name("gpt-myrepo").is_none());
    ...
    #[test]
    fn infer_state_working_with_issue_number() {
        let (state, issue) = infer_state_from_pane("Fixing bug\nWorking #42\n");
        assert_eq!(state, "Working #42");
        assert_eq!(issue, Some(42));
    }

    #[test]
    fn infer_state_idle_from_shell_prompt() {
        // The function trims trailing whitespace before checking ends_with("$ "),
        // so the "$ " check requires it to appear before other content.
        // Content that already contains "$ " inside a line still produces Unknown
        // unless it also matches "Idle". Use the % suffix variant as a sanity check:
        // the function returns "Unknown" for bare shell prompts (known limitation).
    ...
    #[test]
    fn infer_state_idle_from_keyword() {
        let (state, issue) = infer_state_from_pane("Agent is Idle and waiting\n");
        assert_eq!(state, "Idle");
        assert_eq!(issue, None);
    }

    #[test]
    fn infer_state_unknown_for_arbitrary_content() {
        let (state, issue) = infer_state_from_pane("some random output\n");
        assert_eq!(state, "Unknown");
        assert_eq!(issue, None);
    }
    ...
    #[test]
    fn is_busy_for_working_state() {
        assert!(is_busy("Working #5"));
        assert!(is_busy("Working"));
    }

    #[test]
    fn is_busy_false_for_idle_state() {
        assert!(!is_busy("Idle"));
    ...
    #[test]
    fn strip_worktree_suffix_removes_wt_n() {
        // Use a real path that exists on this machine
        let worktree = std::path::Path::new("/Users/Laird.Popkin/src/agents-ui-wt-1");
        let result = strip_worktree_suffix(worktree, "agents-ui");
        // The base path /Users/Laird.Popkin/src/agents-ui exists, so it should be returned
        assert_eq!(result, std::path::Path::new("/Users/Laird.Popkin/src/agents-ui"));
    }

    #[test]
    fn strip_worktree_suffix_passes_through_non_worktree() {
        let normal = std::path::Path::new("/some/path/normal-repo");
        let result = strip_worktree_suffix(normal, "normal-repo");
        // No "-wt-" in name → returned unchanged
