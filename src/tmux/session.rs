...
pub async fn resize_session(transport: &ServerTransport, name: &str, width: u16, height: u16) -> Result<()> {
    ...
    // Keep explicit dimensions to avoid fallback to default tmux widths.
    let _ = transport
        .output(
            "tmux",
            &[
